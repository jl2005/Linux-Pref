...
经过尝试在docker中创建docker方式失败，所以，最后使用VirtualBox安装的虚拟机。
...
```
apt-get install linux-perf linux-tools
```

### execsnoop

[execsnoop](https://github.com/brendangregg/perf-tools/blob/master/execsnoop)使用ftrace 分析exec()的行为，并输出短时进程的行为。
...
1. [清华大学源](https://mirrors.cnnic.cn/help/centos/)


### Ubuntu

1. [清华大学源](https://mirrors.tuna.tsinghua.edu.cn/help/ubuntu/)
2. 163源
...
```
E: Release file for http://security.ubuntu.com/ubuntu/dists/bionic-security/InRelease is not valid yet (invalid for another 13h 8min 6s). Updates for this repository will not be applied.
E: Release file for http://archive.ubuntu.com/ubuntu/dists/bionic-updates/InRelease is not valid yet (invalid for another 13h 9min 9s). Updates for this repository will not be applied.
E: Release file for http://archive.ubuntu.com/ubuntu/dists/bionic-backports/InRelease is not valid yet (invalid for another 13h 10min 32s). Updates for this repository will not be applied.
```

如果执行`apt-get  update`出现这个错误是因为时间不对，可以使用`date`命令修改时间

```
sudo date -s 06/07/20
sudo date -s 11:58:00
```

## 参考

* [Docker容器出现使用systemctl问题：Failed to get D-Bus connection: Operation not permitted](https://plutoacharon.github.io/2020/02/23/Docker容器出现使用systemctl问题：Failed-to-get-D-Bus-connection-Operation-not-permitted/)
* [centos7配置国内yum源](https://blog.csdn.net/xiaojin21cen/article/details/84726193)
* [apt update 提示 Release file for http://… is not valid yet (invalid for another d..)](https://www.cnblogs.com/outs/p/9706437.html)
